...
private constructor(
    ...
    private fun apply(@Suppress("UNUSED_PARAMETER") event: RoundStartedEvent) =
        copy(
                notifications =
    ...
    private fun apply(event: BidPlacedEvent): Result<LobbyState, LobbyErrorCode> {
        ...
        val updatedGameState =
            gameState.apply(event).onFailure {
                return it
            }
        val newNotifications = buildList {
            add(ABidWasPlaced(event.playerId))
            if (updatedGameState.allBidsHaveBeenPlaced)
                add(AllBidsHaveBeenPlaced(updatedGameState.bids))
        }

        return copy(gameState = updatedGameState, notifications = newNotifications).asSuccess()
    }
