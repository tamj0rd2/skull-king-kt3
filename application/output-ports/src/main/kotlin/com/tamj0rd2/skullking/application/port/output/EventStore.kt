...
interface EventStore<ID : AggregateId, E : Event<ID>> {
    fun append(entityId: ID, expectedVersion: Version, events: Collection<E>)
    ...
    fun read(entityId: ID, upToAndIncludingVersion: Version): Collection<E> =
        read(entityId).take(upToAndIncludingVersion.value)

    fun subscribe(subscriber: EventStoreSubscriber<ID, E>)

    companion object {
        fun concurrentModificationException(expectedVersion: Version, actualVersion: Version) =
...
fun interface EventStoreSubscriber<ID : AggregateId, E : Event<ID>> {
    fun onEventReceived(entityId: ID, version: Version)
}
