    ...
    fun `modifying, saving and loading a lobby multiple times results in the same state as just modifying the lobby in memory`() =
        propertyTest {
            checkAll(propertyTestIterations, LobbyCommandArbs.validLobbyCommandsArb) { commands ->
                val lobbyModifiedInMemoryOnly =
                    Lobby.new(PlayerId.random()).also(lobbyRepository::save)
                ...
                expectThat(gameThatWasSavedAndLoaded.allEvents)
                    .isEqualTo(lobbyModifiedInMemoryOnly.allEvents)
                expectThat(gameThatWasSavedAndLoaded.state)
                    .isEqualTo(lobbyModifiedInMemoryOnly.state)
            }
        }

    @Test
    fun `can load a lobby at a specific version`() = propertyTest {
        checkAll(propertyTestIterations, LobbyCommandArbs.lobbyCommandsArb) { commands ->
            ...
            val previousVersionOfLobby =
                lobbyRepository.load(
                    lobby.id,
                    lobbyLoadedAtLatestVersion.loadedAtVersion.previous(),
                )
            expectThat(previousVersionOfLobby.allEvents)
                .isEqualTo(Lobby.from(lobby.allEvents.dropLast(1)).allEvents)
        }
    }
