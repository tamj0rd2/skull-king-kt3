...
class ArchitectureTest {
    ...
    val domainLayerAllowList =
        setOf("java.", "com.tamj0rd2.skullking.", "dev.forkhandles.values.", "strikt.", "org.junit")
    ...
    fun `application ports do not depend on services`() {
        Konsist.scopeFromProject().files.withPackage("$applicationPackage.ports..").assertTrue(
            strict = true
        ) { file ->
    ...
    fun `input ports don't depend on output ports`() {
        Konsist.scopeFromProject()
            .files
            .withPackage("$applicationPackage.ports.input..")
            .assertTrue(strict = true) { file ->
                file.imports.none { import ->
                    import.name.contains("$applicationPackage.ports.output.")
                }
            }
    }
    ...
    fun `output ports don't depend on input ports`() {
        Konsist.scopeFromProject()
            .files
            .withPackage("$applicationPackage.ports.output..")
            .assertTrue(strict = true) { file ->
                file.imports.none { import ->
                    import.name.contains("$applicationPackage.ports.input.")
                }
            }
    }
    ...
    fun `different adapter kinds don't depend on each other`() {
        fun String.adapterSubPackage() =
            this.substringAfter("$adaptersPackage.").substringBefore(".")

        Konsist.scopeFromProject().files.withPackage(adaptersPackageRecursive).assertTrue(
            strict = true
        ) { file ->
            ...
            file.imports
                ...
                .all { importedAdapterKind ->
                    importedAdapterKind in setOf(adapterKind, "configuration")
                }
        }
    }
}
